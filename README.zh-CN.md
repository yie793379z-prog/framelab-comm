...
- 按样本逐条编辑编码字段
- 默认生成本地运行的模拟 AI 编码建议
- 可选接入使用者自己的 OpenAI API Key 来启用真实 AI 建议
...
如果你想启用基于 OpenAI 的真实建议，请在本地创建 `.env.local`：
...
- 当 `AI_SUGGESTION_MODE` 缺失或设置为 `mock` 时，FrameLab 使用本地模拟建议
- 当 `AI_SUGGESTION_MODE=real` 但缺少 `OPENAI_API_KEY` 时，FrameLab 会自动回退到模拟建议
- 当真实 AI 请求失败时，FrameLab 也会自动回退到模拟建议
- API Key 只应保存在本地服务器环境中，不能提交到仓库
- 启用真实 AI 后，当前选中的样本文本会发送到所配置的 AI 提供方
- 使用真实 AI 可能产生 OpenAI API 调用费用，由用户自行承担
